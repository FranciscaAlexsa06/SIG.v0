...
export async function POST(request: Request) {
  try {
    ...
    const [record] = await getDb().insert(workSites).values({ id: `OBR-${crypto.randomUUID().slice(0, 8).toUpperCase()}`, name, company: clean(body.company), client: clean(body.client), address: clean(body.address), status: clean(body.status) || "Activa" }).onConflictDoUpdate({ target: workSites.name, set: { company: clean(body.company), client: clean(body.client), address: clean(body.address), status: clean(body.status) || "Activa", updatedAt: new Date().toISOString() } }).returning();
